...
function MapView(props) {
  ...
  useEffect(() => {
    setLoading(true)
    ImageApi.getMapImages('new', lat, long).then(res => {
      setImageFeed(res)
      setStateImages(res)
      setLoading(false)
    })
  ...
  const generateMarkers = () => {
    ...
    for (let i = 0; i < positions.length; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        if (
          Math.abs(positions[i].lat - positions[j].lat) <= 0.01 &&
          Math.abs(positions[i].long - positions[j].long <= 0.01)
        ) {
          positions[j].zoomLevel = 13
        } else if (
          Math.abs(positions[i].lat - positions[j].lat) <= 0.008 &&
          Math.abs(positions[i].long - positions[j].long <= 0.008)
        ) {
      ...
      if (zoomState >= point.zoomLevel) {
        ...
        return (
          <Marker
            ...
            icon={{
              url: point.img,
              scaledSize: new props.google.maps.Size(scale, scale)
            }}
