...
export default class DisplaySingle extends Component {
  ...
  componentDidMount = () => {
    ...
    const image = this.context.images.find(
      img => img.id === Number(submissionId)
    )

    this.setState({ loading: true })
    CommentApi.getComments(submissionId).then(res => {
      this.setState({
        image,
        comments: res,
        loading: false
  ...
  render = () => {
    ...
    const userIDFromToken =
      TokenService.parseAuthToken() !== undefined
        ? TokenService.parseAuthToken().id
        : ''
        ...
        <div className="DisplaySingle">
          ...
          userLoggedIn && userIDFromToken !== user_id ? (
            <div
              ...
              onClick={() => this.context.incrementUpvotes(id)}
            >
          ...
          <div className="timestamp-display">
            {this.convertTime(create_timestamp)}
          </div>
          ...
          <CommentFeed
            ...
            setCommentsByPush={this.setCommentsByPush}
